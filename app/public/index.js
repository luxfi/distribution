let LUX_SALE        = "0xd0a6e6c54dbc68db5db3a091b171a77407ff7ccf"
...
let getDailyTotals = async () => words32(await call({
  to: LUX_SALE_UTIL, data: sighash("dailyTotals()"),
})).map(web3.toBigNumber)

let getUserBuys = async address => words32(await call({
  to: LUX_SALE_UTIL, data: calldata(sighash("userBuys()"), address),
})).map(web3.toBigNumber)

let getUserClaims = async address => words32(await call({
  to: LUX_SALE_UTIL, data: calldata(sighash("userClaims()"), address),
})).map(Number).map(Boolean)

let getKey = async address => bytes(
  words32(await call({
    to: LUX_SALE, data: calldata(sighash("keys(address)"))
  })).slice(2).map(unhex).join("").replace(/(00)*$/, "")
).map(Number).map(String.fromCharCode).join("")

addEventListener("mousemove", event => lux_ecc.key_utils.addEntropy(
...
let refresh = () => new Promise(resolve => async.parallel(Object.assign({
  ...
  lux_balance: toAsync(balanceOf(LUX_TOKEN, web3.eth.accounts[0])),
  ...
  dailyTotals, eth_balance, lux_balance, publicKey, userBuys, userClaims,
}) => {
  let time = getTime()
  ...
  resolve(update({
    time, days, unclaimed, eth_balance, lux_balance, publicKey
  }))
})))

let render = ({
  time, days, unclaimed, eth_balance, lux_balance, publicKey, buyWindow,
}) => <div>
  <p className="hidden" style={{ width: "95%" }}>

    The LUX Token Distribution will take place over about 341 days.
    1,000,000,000 (one billion) LUX tokens will be created at the
    start of the sale, 100,000,000 LUX are allocated to block.one and cannot be
    transfered.

    The remaining 900,000,000 LUX will be split into different rolling windows of
    availability.  The LUX tokens in a given window will be split
    proportionally to all ETH contributions made during that window.
    200,000,000 LUX will be sold in the first window, lasting five days.
    The remaining 700,000,000 will be divided equally into 350 windows, each
    lasting 23 hours and distributing 2,000,000 LUX.  Contributions can be made to
    any future window, but the LUX cannot be claimed until the window closes.

    Once a window closes, the LUX tokens allocated to that window are
    available to be claimed.

    You must generate and register an LUX Public Key or it will not be possible for
    anyone to include your LUX tokens in the genesis block of any future blockchain's
    based on LUX.IO software.
  ...
  href="https://github.com/luxio/lux-token-distribution">contract source
  code</a>.
  </p>
  ...
  {web3.eth.accounts[0] ? <div>
    <div className="account pane">
      <h2 style={{ textAlign: "center" }}>LUX Token Distribution</h2>
      <div className="info"><table><tbody>
        ...
        <tr>
          <th></th>
          <td style={{ textAlign: "left" }}>
            {formatLUX(unclaimed)} LUX (unclaimed)
          </td>
        </tr>
        <tr>
          <th></th>
          <td style={{ textAlign: "left" }}>
            {formatLUX(lux_balance.div(WAD))} LUX
          </td>
        </tr>
      </tbody></table></div>
        ...
        <div className="row" style={{ marginBottom: "1rem" }}>
          ...
          <div className="button" style={{ marginLeft: "1rem" }}>
            <a href="#" id="generate-link"
              ...
              { publicKey ? "Change your LUX key" : "Generate LUX key"}
            </a>
          </div>
        </div>
        <div className="row">
    ...
    <form className="hidden pane" id="generate-pane"
      ...
      <div id="generate-confirm" className="hidden">
        <h3>{publicKey ? "Change" : "Register"} LUX key</h3>

        {publicKey ? <p>This will replace your LUX claim key:
          <table>
            <tbody>
              <tr>
                <th>Public key</th>
        ...
        <table>
          <tbody>
            <tr>
              <th>Description</th>
              <td style={{ textAlign: "left" }}>
                LUX Token Distribution Claim Key
              </td>
            </tr>
            <tr>
              <th>Public key</th>
        ...
        <p>
          ...
          it right now or you will be unable to access your LUX tokens
          when the sale ends.
    ...
    <div className="hidden pane" id="register-pane">
      <h3>{publicKey ? "Change" : "Register"} LUX public key</h3>
      <table>
        <tbody>
          <tr>
            <th>Public key</th>
    ...
    <form className="hidden pane" id="buy-pane"
      ...
      <table><tbody>
        ...
        <tr>
          <th>LUX Distributed</th>
          <td style={{ textAlign: "left" }}>
            {formatLUX(days[buyWindow].createOnDay)} LUX
          </td>
        </tr>
        <tr>
          <th>Total ETH</th>
        ...
        <tr>
          ...
          <td style={{ textAlign: "left" }}>
            {days[buyWindow].price.toFormat(9)} ETH/LUX
          </td>
        </tr>
        <tr>
    ...
    <form className="hidden pane before-error" id="transfer-pane"
      ...
      <h3>Transfer LUX tokens to another Ethereum account</h3>
      <table><tbody>
        <tr>
        ...
        <tr>
          ...
          <td style={{ textAlign: "left" }}>
            <input placeholder={formatLUX(lux_balance.div(WAD))}
        ...
        <tbody>
          {days.map((day, i) =>
            <tr key={i} className={i == dayFor(time) ? "active" : i < dayFor(time) ? "closed" : ""}>
              ...
              <td>{formatLUX(day.createOnDay)} LUX</td>
              <td>{formatETH(day.dailyTotal)} ETH</td>
              <td>{day.dailyTotal == 0 ? "n/a" : (
                `${day.price.toFormat(9)} ETH/LUX`
              )}</td>
              <td>{day.ends.fromNow()}</td>
              <td>{formatETH(day.userBuys)} ETH</td>
              <td>
                {formatLUX(day.received)} LUX
                {i >= dayFor(time)
                  && <span title="Pending LUX subject to change if additional funds received" style={{ cursor: "pointer" }}> *</span>}
              </td>
            </tr>
          )}
        </tbody>
...
function transfer() {
  ...
  LUXToken.transfer(guy, wad, hopefully(result => ping(result).then(() => {
    hidePanes()
